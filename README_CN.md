...
避免 Release changelog 使用错误的起始版本。自动执行 `patch` 时，脚本会先向 GitHub 请求
release notes，再将返回的 Markdown 写入 changelog 顶部的带日期版本条目。实际执行 `patch`（不是
...
`gh release create` 使用同一份生成的 notes。
脚本不会修改 `example/pubspec.yaml`。需要本机已安装并登录 `gh`；现有按 tag 触发的
workflow 会在脚本已经创建 Release 时自动跳过，避免重复创建。执行 `minor`、`major`
或直接指定版本时，只更新本地根目录 `pubspec.yaml` 和 `CHANGELOG.md`（新增占位条目），并打印 release note 命令供手动执行。
